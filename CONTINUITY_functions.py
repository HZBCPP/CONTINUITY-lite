...
def cluster(slurm_job, cluster_command_line, out_path, ID_path, user_json_file ): 
    ...
    slurm_job_file.write(sys.executable + ' ' +  os.path.realpath(os.path.dirname(__file__)) + "/CONTINUITY_completed_script.py " + user_json_file) 
...
def KWMtoPolyData(SPHARMSurf, SPHARMSurfL, KWMFileName, NAME_PARCELLATION_TABLE):
    ...
    SurfaceWriter.SetFileTypeToASCII() 
...
def compute_point_destrieux(new_parcellation_table, subcorticals_list_checked_with_surfaces, ID):
    ...
    if len(subcorticals_list_checked_with_surfaces) != 0: 
        # Compute the gravity center for each subcortical region: one scalar per region:
...
def subcortical_failed(PPtarget, kernel_size, command):
    print("The subcortical region has failed. Trying morphological closing on PPtarget")
    data, header = nrrd.read(PPtarget)
    kernel = np.ones((kernel_size,kernel_size,kernel_size),np.uint16)
    data = ndimage.binary_closing(data, structure=kernel).astype(np.uint16)
    nrrd.write(PPtarget, data, header)
    ...
    run = subprocess.Popen(command, stdout=0, stderr=subprocess.PIPE)
    out, err = run.communicate()
    print("GenParaMeshCLP", "out: ", colored("\n" + str(out) + "\n", 'green')) 
    print("GenParaMeshCLP", "err: ", colored("\n" + str(err) + "\n", 'red'))
    print("The return code is: ", run.returncode)
...
def generating_subcortical_surfaces(OUT_FOLDER, ID, labeled_image, Labels, LabelNames, 
    ...
    while index < len(Labels) :  
        if Labels[index] != 0: 
            ...
            PPtarget = os.path.join(OutputDir, ID + '-T1_SkullStripped_scaled_label_' + LabelNames[index] +'_pp.nrrd')
            ...
            if os.path.exists(Paratarget):
                ...
                kernel_size = 1
                while return_code!=0:
                    return_code = subcortical_failed(PPtarget, kernel_size, command)
                    kernel_size += 1

                    #if kernel_size>20:
                        #print("The kernel size exceeded 20x20x20")
                        #break
