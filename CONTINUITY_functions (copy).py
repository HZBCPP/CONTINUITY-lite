    ...
    while index < len(Labels) :  
        if Labels[index] != 0: 
            ...
            if os.path.exists(Paratarget):
                ...
                if run.returncode != 0:
                    print("The subcortical region has failed. Trying morphological opening on PPtarget")
                    data, header = nrrd.read(PPtarget)
                    kernel = np.ones((3,3,3),np.uint16)
                    data = scipy.ndimage.binary_opening(data, structure=kernel).astype(np.uint16)
